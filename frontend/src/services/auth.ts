/**
 * @fileoverview Сервис авторизации для фронтенда (Telegram Mini App).
 *
 * Отвечает за:
 * - Взаимодействие с API авторизации (`login`, `refresh`)
 * - Хранение токенов и данных пользователя в localStorage
 * - Управление жизненным циклом сессии и синхронизацию с `apiClient` (см. `api.ts`)
 *
 * Токены:
 * - **Access token** (JWT) — для заголовка `Authorization` при запросах к API
 * - **Refresh token** (opaque) — для `POST /auth/refresh`; ротация на сервере
 */
...
class AuthService {
   ...
   * Старый refresh token на сервере инвалидируется (ротация), возвращается новая пара.
   *
   * @returns Новая пара токенов или `null` при ошибке (сессия очищается)
   */
  ...
  /**
   * Восстанавливает сессию из localStorage: подставляет access token в API-клиент.
   *
   * @returns `true`, если есть сохранённый refresh token (сессию можно попробовать обновить через `refresh()`)
   */
...
/** Один refresh на 401 во всех запросах `apiClient` (см. `api.ts`). */
apiClient.setRefreshHandler(() =>
  authService.refresh().then((tokens) => tokens !== null),
);
